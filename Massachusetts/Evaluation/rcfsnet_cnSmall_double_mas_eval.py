...
from RCFSNet_ConvNeXt_Small_NoSigmoid import RCFSNet_CN_Small_NoSigmoid

source = '../../massachusetts-roads-dataset/tiff/test/images'
...
solver = TTAFrame(RCFSNet_CN_Small_NoSigmoid)

solver.load('../Training/weights/RCFSNet_mas_CnSmall_double_pos_060_v01_100epoch.th')

target = 'submits/RCFSNet_CnSmall_double_Mas_pos_060_v01/'

tic = time()
if not os.path.exists(target):
    os.makedirs(target)    

gt_root = '../../massachusetts-roads-dataset/tiff/test/masks'
...
#     mask = cv2.erode(mask, kernel, iterations=1) 
    ...
    new_mask = np.concatenate([new_mask[:, :, None] * 255, new_mask[:, :, None] * 255, new_mask[:, :, None] * 255], axis=2)
    
    cv2.imwrite(target + name.split('.')[0] + '_pre-mask.png', new_mask.astype(np.uint8))
...
df.to_csv('rcfsnet_CnSmall_double_test_Mas_pos_120_v21.csv')