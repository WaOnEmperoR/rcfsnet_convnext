...
class RCFSNet_CN_Tiny_NoSigmoid_ScaleSenMod(nn.Module):
    def __init__(self, num_classes=1, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,
                 base_channel=32):
        super(RCFSNet_CN_Tiny_NoSigmoid_ScaleSenMod, self).__init__()
...
#         resnet = models.resnet34(pretrained=False)
        ...
        self.weights = nn.Parameter(torch.ones(5), requires_grad=True)  # 5 scales
        ...
        self.conv_flat1 = nn.Conv2d(filters[0], 1, kernel_size=1, stride=1, padding=0)
        
        self.conv_flat2 = nn.Conv2d(5, 1, kernel_size=1, stride=1, padding=0)
        
#         self.conv1024 = nn.Conv2d(in_channels=2048, out_channels=1024,kernel_size=3,
#                        stride=1,padding=1)
#         self.conv512 = nn.Conv2d(in_channels=1024, out_channels=512, kernel_size=3,
#                          stride=1, padding=1)
#         self.conv256 = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=3,
#                          stride=1, padding=1)
#         self.conv160 = nn.Conv2d(in_channels=320, out_channels=160, kernel_size=3,
#                           stride=1, padding=1)
...
#         print('end MSCE')
        ...
        sc5 = self.conv5(self.hd5_d0(hd5))        
        sc4 = self.conv4(self.hd4_d0(d4))        
        sc3 = self.conv3(self.hd3_d0(d3))        
        sc2 = self.conv2(self.hd2_d0(d2))
        sc1 = self.d1_d0(d1)
        
#         print(f"SC5 shape: {sc5.shape}")
#         print(f"SC4 shape: {sc4.shape}")
#         print(f"SC3 shape: {sc3.shape}")
#         print(f"SC2 shape: {sc2.shape}")
#         print(f"SC1 shape: {sc1.shape}")
                
        sc5_flat = self.conv_flat1(sc5) * self.weights[4]
        sc4_flat = self.conv_flat1(sc4) * self.weights[3]
        sc3_flat = self.conv_flat1(sc3) * self.weights[2]
        sc2_flat = self.conv_flat1(sc2) * self.weights[1]
        sc1_flat = self.conv_flat1(sc1) * self.weights[0]
        
        concat_feature = torch.cat([sc5_flat, sc4_flat, sc3_flat, sc2_flat, sc1_flat], dim=1)
        
        return self.conv_flat2(concat_feature)
        


#         sum_shape = (self.weights[0] * d1) + (self.weights[1] * sc2) + (self.weights[2] * sc3) + (self.weights[3] * sc4) + (self.weights[4] * sc5)
        
#         main_out = self.main_head(sum_shape)
        
#         return main_out
    
#         return ((self.weights[0] * sc1_flat) + (self.weights[1] * sc2_flat) + (self.weights[2] * sc3_flat) + (self.weights[3] * sc4_flat))
...
def lunwen3():
    return RCFSNet_CN_Tiny_NoSigmoid()
...
if __name__ == "__main__":
    ...
    RCFSNet_CN_Tiny_NoSigmoid_ScaleSenMod = RCFSNet_CN_Tiny_NoSigmoid_ScaleSenMod()
    predicted = RCFSNet_CN_Tiny_NoSigmoid_ScaleSenMod(a)
