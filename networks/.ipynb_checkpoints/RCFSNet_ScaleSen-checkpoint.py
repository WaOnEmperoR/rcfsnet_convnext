...
# #         self.w_numpy = np.array([1.5, 1.0, 1.0, 2.0, 1.0])
#         self.weights = nn.Parameter(torch.from_numpy(np.array([1.0, 2.0, 1.0, 1.0, 1.5])), requires_grad=True)
...
#         self.main_head = BaseNetHead(filters[0], num_classes, 1,
#                                      is_aux=False, norm_layer=norm_layer)
...
#         print('h5:', h5.shape)
        ...
        sc5 = self.conv5(self.hd5_d1(hd5)) # from MSCE       
...
#         sum_shape = (d1) + (2.5 * sc2) + (sc3)  + (sc4) + (1.5 * sc5)
       
        main_out = self.main_head(sum_shape)
        
        return F.sigmoid(main_out)       
...
if __name__ == "__main__":
    ...
    RCFSNet_ScaleSen = RCFSNet_ScaleSen()
    predicted = RCFSNet_ScaleSen(a)
