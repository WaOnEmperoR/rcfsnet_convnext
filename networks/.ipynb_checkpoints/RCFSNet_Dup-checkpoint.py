...
#         self.ConvBnRelu = ConvBnRelu(in_planes=512)
        ...
        self.stem = convnext_tiny.features[0]
        self.stem_upsample = nn.ConvTranspose2d(96, 96, 2, stride=2)
        
        self.stage1 = convnext_tiny.features[1]
        self.maxpool1 = convnext_tiny.features[2]
        self.stage2 = convnext_tiny.features[3]
        self.maxpool2 = convnext_tiny.features[4]
        self.stage3 = convnext_tiny.features[5]
        self.maxpool3 = convnext_tiny.features[6]
        self.stage4 = convnext_tiny.features[7]
...
#         self.decoder2 = DecoderBlock(filters[-4], filters[-4], relu=False)  # 32
        ...
        self.decoder5 = DecoderBlock(filters2[-1], filters2[-2], relu=False, last=True)  # 384
        self.decoder4 = DecoderBlock(filters2[-2], filters2[-3], relu=False)  # 192
        self.decoder3 = DecoderBlock(filters2[-3], filters2[-4], relu=False)  # 96
        self.decoder2 = DecoderBlock(filters2[-4], filters2[-4], relu=False)  # 96
        ...
        self.main_head = BaseNetHead(filters2[0], num_classes, 2,
                                     is_aux=False, norm_layer=norm_layer)
        
        self.conv_1x1_3x3 = nn.Sequential(
                ConvBnRelu(96, 32, 1, 1, 0,
...
#                            has_bn=True, norm_layer=norm_layer,
        ...
        self.conv_1x1_2 = nn.Conv2d(32, 1, kernel_size=1,stride=1, padding=0)
...
#                                stride=1,padding=1)
        self.conv5 = nn.Conv2d(in_channels=filters2[-1],out_channels=filters2[1],kernel_size=3,
                               stride=1,padding=1)
        self.conv4 = nn.Conv2d(in_channels=filters2[-2],out_channels=filters2[1],kernel_size=3,
                               stride=1,padding=1)
        self.conv3 = nn.Conv2d(in_channels=filters2[-3],out_channels=filters2[1],kernel_size=3,
                               stride=1,padding=1)
        self.conv2 = nn.Conv2d(in_channels=filters2[-4],out_channels=filters2[1],kernel_size=3,
                               stride=1,padding=1)
        self.relu = nn.ReLU()
        ...
        self.conv512_768 = nn.Conv2d(in_channels=512, out_channels=768,kernel_size=3,
                               stride=1,padding=1)
        self.conv256_384 = nn.Conv2d(in_channels=256, out_channels=384,kernel_size=3,
                               stride=1,padding=1)
        self.conv128_192 = nn.Conv2d(in_channels=128, out_channels=192,kernel_size=3,
                               stride=1,padding=1)
        self.conv64_96 = nn.Conv2d(in_channels=64, out_channels=96,kernel_size=3,
                               stride=1,padding=1)
    ...
    def forward(self, inputs):
        ...
        if not h1.is_contiguous():
            print ("h1 is not cont")            
        else:
            print("h1 is cont")
            
        if not h1_up.is_contiguous():
            print ("h1_up is not cont")
        else:
            print("h1_up is cont")
            
        if not h2.is_contiguous():
            print ("h2 is not cont")
        else:
            print("h2 is cont")
        
        if not h3.is_contiguous():
            print ("h3 is not cont")
        else:
            print("h3 is cont")
...
#         hd5 = self.conv512_768(hd5)
#         h4 = self.conv256_384(h4)
#         h3 = self.conv128_192(h3)
#         h2 = self.conv64_96(h2)
#         h1 = self.conv64_96(h1)

#         d4 = self.relu(self.conv256(torch.cat([self.decoder5(hd5), m4], dim=1)))  # 256  64   1
        print(h4.shape)
        d4 = self.relu(self.conv384(torch.cat([self.decoder5(hd5), h4], dim=1)))  # 256  64   1
#         d3 = self.relu(self.conv128(torch.cat([self.decoder4(d4), m3], dim=1)))  # 256  64   1
        print(h3.shape)
        d3 = self.relu(self.conv192(torch.cat([self.decoder4(d4), h3], dim=1)))  # 256  64   1
#         d2 = self.relu(self.conv64_1(torch.cat([self.decoder3(d3), m2], dim=1)))  # 256  64   1
        print(h2.shape)
        d2 = self.relu(self.conv96_1(torch.cat([self.decoder3(d3), h2], dim=1)))  # 256  64   1
        print(h1_up.shape)
        d1 = self.relu(self.conv96_2(torch.cat([self.decoder2(d2), h1_up], dim=1)))  # 256  64   1
        
        if not d1.is_contiguous():
            print ("d1 is not cont")
        else:
            print("d1 is cont")
            
        full_concat = d1 + self.conv5(self.hd5_d1(hd5)) + self.conv4(self.hd4_d1(d4)) + self.conv3(
                self.hd3_d1(d3)) + self.conv2(self.hd2_d1(d2))
        
        print("====FC-shape====")
        print(full_concat.shape)
        
        main_out = F.sigmoid(self.main_head(full_concat))
