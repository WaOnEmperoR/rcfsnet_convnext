...
class CDAM2(nn.Module):
    ...
    def __init__(self, k_size=9):
        ...
        self.convout = nn.Conv2d(256 * 5 * 4, 256*5, kernel_size=3, padding=1, bias=False)
        self.conv111 = nn.Conv2d(in_channels=256*5*2, out_channels=256*5*2, kernel_size=1, padding=0, stride=1)
        self.conv222 = nn.Conv2d(in_channels=256*5*2, out_channels=256*5*2, kernel_size=1, padding=0, stride=1)
...
class CDAM3(nn.Module):
    ...
    def __init__(self, k_size=7):
        ...
        self.conv22 = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
        self.convout = nn.Conv2d(256*4*5, 256*5, kernel_size=3, padding=1, bias=False)
        self.conv111 = nn.Conv2d(in_channels=256*5*2, out_channels=256*5*2, kernel_size=1, padding=0, stride=1)
        self.conv222 = nn.Conv2d(in_channels=256*5*2, out_channels=256*5*2, kernel_size=1, padding=0, stride=1)
...
#         resnet = models.resnet34(pretrained=False)
        ...
        self.main_head = BaseNetHead(160, num_classes, 2,
                                     is_aux=False, norm_layer=norm_layer)
        self.conv5 = nn.Conv2d(in_channels=filters[-1],out_channels=160,kernel_size=3,
                               stride=1,padding=1)
        self.conv4 = nn.Conv2d(in_channels=filters[-2],out_channels=160,kernel_size=3,
                               stride=1,padding=1)
        self.conv3 = nn.Conv2d(in_channels=filters[-3],out_channels=160,kernel_size=3,
                               stride=1,padding=1)
        self.conv2 = nn.Conv2d(in_channels=filters[-4],out_channels=160,kernel_size=3,
                               stride=1,padding=1)
        self.relu = nn.ReLU()
...
#         print('h5:', h5.shape)
        ...
        # d5=d_bottom+c5           #512
        d4 = self.relu(self.conv1024(torch.cat([self.decoder5(hd5), m4], dim=1)))  # 256  64   1
        d3 = self.relu(self.conv512(torch.cat([self.decoder4(d4), m3], dim=1)))  # 256  64   1
        d2 = self.relu(self.conv256(torch.cat([self.decoder3(d3), m2], dim=1)))  # 256  64   1
        d1 = self.relu(self.conv160(torch.cat([self.decoder2(d2), h1], dim=1)))  # 256  64   1
...
if __name__ == "__main__":
    ...
    RCFSNet_RN50 = RCFSNet_RN50()
    predicted = RCFSNet_RN50(a)
